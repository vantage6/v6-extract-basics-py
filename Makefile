...
# When true, also tag/push ${REGISTRY}/algorithm/extract-basics:${VANTAGE6_MAJOR}. CI sets false on
...
image:
	@set -e; \
	echo "Building ${REGISTRY}/algorithm/extract-basics:${TAG}-v6-${VANTAGE6_VERSION}"; \
	echo "Building ${REGISTRY}/algorithm/extract-basics:latest"; \
	EXTRA_MAJOR=""; \
	if [ "$(INCLUDE_V6_MAJOR_TAG)" = true ]; then \
	  echo "Building ${REGISTRY}/algorithm/extract-basics:${VANTAGE6_MAJOR}"; \
	  EXTRA_MAJOR='--tag ${REGISTRY}/algorithm/extract-basics:${VANTAGE6_MAJOR}'; \
	fi; \
	docker buildx build \
		--tag ${REGISTRY}/algorithm/extract-basics:${TAG}-v6-${VANTAGE6_VERSION} \
		--tag ${REGISTRY}/algorithm/extract-basics:latest \
		$$EXTRA_MAJOR \
		--platform ${PLATFORMS} \
		-f ./Dockerfile \
